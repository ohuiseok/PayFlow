...
$transferCount = [int](Invoke-MySql "SELECT COUNT(*) FROM $transferDb.transfers WHERE idempotency_key LIKE '$keyPattern';")
$duplicateIdempotency = [int](Invoke-MySql "SELECT COUNT(*) FROM (SELECT idempotency_key FROM $transferDb.transfers WHERE idempotency_key LIKE '$keyPattern' GROUP BY idempotency_key HAVING COUNT(*) > 1) duplicated;")
$duplicateWalletTransactions = [int](Invoke-MySql "SELECT COUNT(*) FROM (SELECT wt.wallet_id, wt.transaction_type, wt.reference_type, wt.reference_id FROM $walletDb.wallet_transactions wt JOIN $transferDb.transfers t ON wt.reference_id = CAST(t.id AS CHAR) WHERE t.idempotency_key LIKE '$keyPattern' AND wt.reference_type = 'TRANSFER' GROUP BY wt.wallet_id, wt.transaction_type, wt.reference_type, wt.reference_id HAVING COUNT(*) > 1) duplicated;")
$succeededTransactionAnomalies = [int](Invoke-MySql "SELECT COUNT(*) FROM (SELECT t.id, SUM(CASE WHEN wt.transaction_type = 'WITHDRAW' THEN 1 ELSE 0 END) withdrawals, SUM(CASE WHEN wt.transaction_type = 'DEPOSIT' THEN 1 ELSE 0 END) deposits FROM $transferDb.transfers t LEFT JOIN $walletDb.wallet_transactions wt ON wt.reference_id = CAST(t.id AS CHAR) AND wt.reference_type = 'TRANSFER' WHERE t.idempotency_key LIKE '$keyPattern' AND t.status = 'SUCCEEDED' GROUP BY t.id HAVING withdrawals <> 1 OR deposits <> 1) anomalies;")
$missingOutbox = [int](Invoke-MySql "SELECT COUNT(*) FROM $transferDb.transfers t LEFT JOIN $transferDb.outbox_events o ON o.event_key = CAST(t.id AS CHAR) WHERE t.idempotency_key LIKE '$keyPattern' AND o.id IS NULL;")
$unpublishedOutbox = [int](Invoke-MySql "SELECT COUNT(*) FROM $transferDb.transfers t JOIN $transferDb.outbox_events o ON o.event_key = CAST(t.id AS CHAR) WHERE t.idempotency_key LIKE '$keyPattern' AND o.status <> 'PUBLISHED';")
$missingSucceededLedger = [int](Invoke-MySql "SELECT COUNT(*) FROM $transferDb.transfers t LEFT JOIN $ledgerDb.ledger_entries le ON le.transfer_id = t.id WHERE t.idempotency_key LIKE '$keyPattern' AND t.status = 'SUCCEEDED' AND le.id IS NULL;")
$duplicateLedger = [int](Invoke-MySql "SELECT COUNT(*) FROM (SELECT le.transfer_id FROM $ledgerDb.ledger_entries le JOIN $transferDb.transfers t ON t.id = le.transfer_id WHERE t.idempotency_key LIKE '$keyPattern' GROUP BY le.transfer_id HAVING COUNT(*) > 1) duplicated;")
$negativeBalances = [int](Invoke-MySql "SELECT COUNT(*) FROM $walletDb.wallets WHERE balance < 0;")
$compensationRequired = [int](Invoke-MySql "SELECT COUNT(*) FROM $transferDb.transfers WHERE idempotency_key LIKE '$keyPattern' AND status = 'COMPENSATION_REQUIRED';")
...
Add-QueryEvidence 'Transfer status summary' "SELECT status, COUNT(*) AS count, COALESCE(SUM(amount), 0) AS amount_sum FROM $transferDb.transfers WHERE idempotency_key LIKE '$keyPattern' GROUP BY status ORDER BY status;"
Add-QueryEvidence 'Outbox status summary' "SELECT o.status, COUNT(*) AS count FROM $transferDb.transfers t JOIN $transferDb.outbox_events o ON o.event_key = CAST(t.id AS CHAR) WHERE t.idempotency_key LIKE '$keyPattern' GROUP BY o.status ORDER BY o.status;"
