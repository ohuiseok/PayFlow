...
| settlement-service | Toss 승인/취소 수집, 일별 집계, 원장 대사 | 지갑 또는 원장 직접 변경 |
...
-> Kafka payment.settlement
-> settlement-service settlement_transactions
-> daily Spring Batch
-> ledger-service payment-entry 조회
-> settlement_items + settlement_runs
```

정산 consumer는 `event_id` unique로 중복을 막는다. 배치는 원장 누락과 금액 불일치를 실패로 숨기지 않고 `WITH_DISCREPANCY`와 거래별 사유로 남긴다.
