...
## 6. 정산 배치는 성공했지만 원장 차이가 남는 문제

### Problem

Toss 정산 이벤트는 수집됐지만 해당 결제 원장이 없거나 금액이 달라 배치 결과가 정확하지 않을 수 있습니다.

### Decision

배치 자체를 예외로 끝내지 않고 거래별로 `MISSING_LEDGER`, `AMOUNT_MISMATCH`를 저장하고 기준일 실행을 `WITH_DISCREPANCY`로 표시합니다.

### Result

집계 성공과 대사 성공을 구분해 운영자가 차이가 있는 거래를 추적할 수 있습니다.

### Lesson

배치가 기술적으로 완료됐다는 사실과 금전 데이터가 일치한다는 사실은 다릅니다.
