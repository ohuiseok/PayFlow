...
| 데이터베이스 | MySQL 8.4 |
| 캐시/락 | Redis 7 |
| 메시징 | Apache Kafka 3.8 |
| 진입점 | Nginx/API Gateway |
...
| 지갑 조회 | `GET /api/wallets/users/{userId}` |
| 은행 충전 | `POST /api/bank/deposits`, `GET /api/bank/transfers/{id}` |
| 송금 | `POST /api/transfers`, `GET /api/transfers/{id}` |
| 미션 지원금 | `PATCH /api/missions/{id}/approve`, `POST /api/missions/{id}/pay` |
| 아웃박스 모니터링 | `GET /api/transfers/outbox/summary` |
