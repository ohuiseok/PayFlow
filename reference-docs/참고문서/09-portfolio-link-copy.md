...
Gateway 라우팅, 인증, 멱등성 헤더, 지갑/송금/미션/오픈뱅킹 API 계약을 정리했습니다.

### ERD

서비스별 독립 DB, 지갑 거래 이력, 송금, outbox, 원장 테이블과 주요 제약 조건을 정리했습니다.
...
PayFlow는 부모-자녀 미션 보상 도메인을 기반으로 지갑 잔액 정합성, 멱등성, 송금 실패 복구, Kafka 기반 원장 기록을 구현한 MSA 포트폴리오 프로젝트입니다.

## 3-Line Version

PayFlow는 부모가 자녀에게 미션을 부여하고 보상금을 지급하는 지갑 서비스입니다.
...
- Spring Boot MSA 기반 부모-자녀 미션 보상 지갑 서비스 구현
