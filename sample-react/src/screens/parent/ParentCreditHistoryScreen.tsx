...
export function ParentCreditHistoryScreen(_props: Props) {
  ...
  return (
    <ScreenFrame eyebrow="예산 기록" title="최근 지원금 예산 기록" description="지원금 충전 및 정책 미션 지급 내역입니다.">
      <ApiErrorBox error={entriesQuery.error} fallback="지원금 예산 기록 조회에 실패했습니다." />
      {entriesQuery.isLoading ? (
        <LoadingState title="불러오는 중" body="지원금 예산 기록을 불러오고 있습니다." />
      ) : displayEntries.length === 0 ? (
        <EmptyState body="지원금 예산 기록이 없습니다." />
      ) : (
