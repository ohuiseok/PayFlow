...
PayFlow의 ledger consumer는 원장 source unique 제약, settlement consumer는 `event_id` unique 제약으로 offset 재처리 중복을 방어한다.

### PayFlow에서 확인할 위치

ledger/settlement consumer config, 원장 source unique key, settlement event_id unique key, Kafka commit logs
