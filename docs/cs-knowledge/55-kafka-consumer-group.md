...
`ledger-service`와 `settlement-service`는 같은 `transfer.completed` Topic을 읽더라도 서로 다른 Consumer Group을 사용해야 한다. 그래야 두 서비스가 모두 같은 이벤트를 받을 수 있다.
...
ledger-service와 settlement-service가 같은 group id를 써서 둘 중 하나만 이벤트를 받는다.
...
```text
Kafka Consumer Group 개념은 PayFlow에서 ledger-service와 settlement-service가 같은 이벤트를 각각 독립적으로 받게 하기 위해 필요하다.
이 개념이 없으면 두 서비스가 같은 group id를 쓰면 한쪽만 transfer.completed 이벤트를 받아 다른 한쪽 처리가 누락된다.
