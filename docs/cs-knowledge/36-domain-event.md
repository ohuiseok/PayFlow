...
banking-service --[HTTP POST]--> settlement-service

// producer가 consumer 주소와
// API 스펙을 직접 알아야 한다
```
...
banking-service --> [PaymentSettlement 이벤트 발행] --> Kafka
                                                            ↓
...
PayFlow에서는 두 도메인 이벤트를 구분한다. `transfer-service`의 `TransferCompleted`는 `ledger-service`가 소비하고, `banking-service`의 Toss 승인/취소 이벤트는 `settlement-service`가 소비한다.
...
송금 완료는 원장에, Toss 승인/취소는 정산에 전달해야 하는데 직접 호출만 쓰면 서비스 간 결합이 커진다.
...
```text
Domain Event 개념은 PayFlow에서 송금 완료를 원장에, Toss 승인/취소를 정산 서비스에 느슨하게 전달하기 위해 필요하다.
