...
PayFlow에서는 `transfer-service`가 송금 완료 이벤트를 Kafka로 발행하고, `ledger-service`와 `settlement-service`가 이를 소비한다.
...
~~~text
transfer.completed topic으로 후속 처리를 분리하고 ledger/settlement가 독립 consumer group으로 소비한다.
~~~
...
infrastructure/kafka/topics.md, transfer publisher, ledger consumer
