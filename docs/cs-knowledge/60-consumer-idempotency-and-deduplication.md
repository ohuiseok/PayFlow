...
`ledger-service`는 원장의 `(source_type, source_id)` unique 제약으로 같은 송금 이벤트를 중복 기록하지 않는다. `settlement-service`는 사전 `event_id` 조회와 `settlement_transactions.event_id` unique 제약을 함께 사용하고, 동시 INSERT 경합의 `DataIntegrityViolationException`도 중복으로 처리한다.

PayFlow는 공통 `processed_event` 테이블이 아니라 각 도메인의 식별자와 유니크 제약을 최종 방어선으로 사용한다.
...
ledger consumer와 원장 source unique key, settlement consumer와 `settlement_transactions.event_id`
...
```text
Consumer Idempotency And Deduplication 개념은 PayFlow에서 같은 송금 이벤트나 Toss 정산 이벤트가 여러 번 와도 각 결과가 한 번만 반영되게 하기 위해 필요하다.
이 개념이 없으면 Consumer 재시작 후 같은 이벤트를 다시 읽어 차변/대변 원장이 중복 생성될 수 있다.
그래서 코드에서는 원장 source와 정산 eventId 유니크 제약, 처리와 기록의 같은 트랜잭션을 적용하고,
