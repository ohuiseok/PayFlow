...
PayFlow에는 두 Outbox가 있다. `transfer-service.outbox_events`는 송금 완료/실패 이벤트를 원장으로 전달하고, `banking-service.payment_settlement_outbox`는 Toss 승인/취소 이벤트를 정산 서비스로 전달한다. 두 경우 모두 도메인 상태와 발행 의도를 같은 DB 트랜잭션에 저장한다.

transfer outbox는 원자적 claim, stale 복구, 모니터링 API가 있다. banking settlement outbox는 최대 50건 조회와 제한 재시도는 있지만 원자적 claim, stale 복구, 모니터링 API는 아직 없다.
...
transfer outbox/relay, banking `payment_settlement_outbox`/relay, Kafka producer
...
- 송금 완료 또는 Toss 승인/취소가 커밋됐지만 Kafka 발행 전에 서버가 죽으면 원장 또는 정산 수집이 누락될 수 있다.
- 각 도메인 상태와 outbox row를 같은 DB 트랜잭션에 저장하고 relay가 미발행 이벤트를 반복 발행한다.
