...
`ledger-service`에는 제한 재시도와 DLT가 설정되어 있다. 반면 `settlement-service`의 `PaymentSettlementEventConsumer`는 파싱 예외를 전파하지만 별도 error handler/DLT가 없다.

따라서 잘못된 `payment.settlement` payload가 후속 정산 이벤트 처리를 막지 않도록 같은 격리 정책을 추가해야 한다.
...
ledger Kafka error handler/DLT, settlement consumer 설정, DLT alerting
...
그래서 ledger에는 제한 재시도와 DLT를 적용하고, settlement에도 같은 격리 정책과 재처리 도구를 추가해야 하며,
