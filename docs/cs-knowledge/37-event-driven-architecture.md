...
banking-service  --[HTTP 직접 호출]--> settlement-service

// EDA 구조
transfer-service --> [TransferCompleted] --> Kafka --> ledger-service
banking-service  --> [PaymentSettlement] --> Kafka --> settlement-service
```
...
ledger-service Consumer Group (`transfer.completed`):
  consumer-1이 Partition 0 소비 (offset 0, 1, 2, ...)

settlement-service Consumer Group (`payment.settlement`):
...
PayFlow에서 송금 완료 후 원장 기록과 Toss 승인/취소 후 정산 수집은 서로 다른 이벤트 흐름으로 분리한다.
...
이렇게 하면 `transfer-service`는 원장 구현을, `banking-service`는 정산 구현을 몰라도 된다.
...
```text
Event Driven Architecture 개념은 PayFlow에서 송금-원장과 Toss-정산 후속 처리를 비동기로 분리하면서 신뢰성을 유지하기 위해 필요하다.
이 개념이 없으면 이벤트 유실이나 중복 소비로 송금-원장 또는 Toss-정산 상태가 어긋날 수 있다.
