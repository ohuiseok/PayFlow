...
| 용돈 지급 내역을 추적하기 어렵다 | 지갑 거래 이력과 사용 기록으로 돈의 흐름을 조회한다 |
...
14. 부모와 자녀는 지갑 거래 이력과 사용 기록에서 결과를 확인한다.
```
...
- 사용 기록 전용 테이블은 만들지 않고 미션과 지갑 거래 데이터를 조합해 조회한다.
...
| reward-service | 부모-자녀 연결, 미션 상태 관리, 보상 지급 요청, 사용 기록 조회 |
