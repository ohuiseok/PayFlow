...
초기에는 테스트베드 또는 mock 값으로 계좌 식별자를 등록한다. 부모 충전과 자녀 출금 모두 같은 BankAccount 모델을 사용하되, 요청 사용자 본인 소유 계좌만 사용할 수 있다.
...
PayFlow 지갑 잔액을 사용자 본인의 은행 계좌로 출금한다. 자녀 화면에서는 `13-child-withdrawal.svg`에서 미션 보상으로 받은 지갑 잔액을 연결 계좌로 보내는 흐름이다.
...
```text
- X-User-Id 사용자는 walletId 소유자여야 한다.
- X-User-Id 사용자는 bankAccountId 소유자여야 한다.
- 부모가 연결된 자녀 지갑을 조회할 수 있더라도 부모 계좌로 자녀 잔액을 출금할 수 없다.
- bankAccount는 ACTIVE 상태여야 하며 입금이체에 필요한 외부 식별자를 보유해야 한다.
```
...
1. Idempotency-Key header 확인
2. 요청 body hash 계산
3. Wallet 소유권, BankAccount 소유권과 상태 확인
...
13. 자녀 화면은 PROCESSING/COMPLETED 상태를 캐시북 출금 기록으로 표시
```
...
출금 요청/결과 조회 API가 `13-child-withdrawal` 화면과 연결 가능한 응답을 반환
```
