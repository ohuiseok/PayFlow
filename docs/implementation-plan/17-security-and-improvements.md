...
### 3-D. getParentCreditSummary COUNT/SUM 쿼리 최적화 [M-4]

**영향 파일**: `reward-service/.../RewardService.java`, `RewardTaskRepository.java`

```java
// 전체 목록 로드 후 size() → COUNT 쿼리로 교체
long pendingApprovalCount = rewardTaskRepository.countByParentUserIdAndStatus(requestUserId, SUBMITTED);

// 전체 목록 메모리 필터링 → DB SUM으로 교체
BigDecimal monthlyPaid = rewardTaskRepository.sumRewardAmountByParentAndStatusAndMonth(
