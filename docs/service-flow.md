...
부모와 자녀가 각각 계정을 만들고, 역할에 맞는 화면과 권한을 갖습니다.
...
- 같은 충전 요청이 중복 반영되지 않도록 `referenceId`를 사용합니다.
...
- 보상 금액은 부모 지갑 잔액 이하로 제한하는 정책을 권장합니다.
...
- 사진 업로드는 서버 직접 업로드보다 업로드 URL 발급 방식을 권장합니다.
...
| 알림 | reward-service 내부 notification 모듈, 후속 분리 시 notification-service |
| 인증 사진 파일 URL | reward-service 내부 file 모듈, 후속 분리 시 file-service |
...
|---|---|
| 크레딧 충전 반영 | `chargeId` |
| 미션 승인/보상 지급 | `reward-payment-mission-{missionId}` |
...
1. user-service 역할 확장
2. wallet-service 지갑 생성/잔액 확인 안정화
3. 가족 연결 API
4. 부모 크레딧 충전 API
5. 미션 등록/조회/제출/승인/반려 API
6. 보상 지급과 멱등성
7. 캐시북 기록
8. 알림
9. 원장/정산
